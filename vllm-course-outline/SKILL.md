...
本 skill 的默认教学设计原则已经升级为：

- `单课单主题`
- `主课程讲主线，专题课程讲深水区`
- `同一课时只保留一个重要主题`
- `如果一个标题里出现多个不同问题域，应优先拆课，而不是并课`
...
8. 对于每一章或每一课，都说明：
   ...
   - 推荐先看哪些文件
   - 和前后内容如何衔接
9. 如果用户提到“最新版本”“当前版本”“v0.19.0”之类的版本化要求，必须先核对官方 release 信息，再写课纲。对版本相关结论要显式写明：
   - 版本号
   - 官方发布日期
   - 本课纲中哪些专题是该版本特别值得新增或强化的内容
10. 如果用户要求“20 节课程”或“深度版”，也不要默认把多个不同主题塞进同一课；深度版的首要原则仍然是“单课单主题”，其次才是凑齐课时数。
...
- 编译与 kernel 优化
- Weight Offloading 与 KV Offloading
- PD 分离
- Structured Outputs 与工具调用
- Attention Backend 体系
- 重要模型结构
...
如果用户要求“严格单课单主题”，优先输出 `标准版主课程 + 专题高阶课程组合方案`，而不是单独的 20 课母版。
如果用户只说“扩展到 20 节课”，默认理解为“用主课程加专题营拼成 20 节”，而不是把多个主题压回单一主课。
...
如果用户只要求“做成深度版”，但没有指定必须正好 20 节，则优先生成：

- `15 节标准版主课程 + 1 个 3 节专题营`
  或
- `15 节标准版主课程 + 1 个 4 节专题营`
  或
- `15 节标准版主课程 + 1 个 5 节专题营`

只有当用户明确要求“连续 20 节单一主课”或“20 节母版目录”时，才输出连续 20 节目录，并额外说明这更适合作为母版而不是最优授课结构。
